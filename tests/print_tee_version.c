...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
			printf(
				"You can pass the path to the TEE device to query, otherwise "
	...
	if (rc) {
		fprintf(stderr, "failed to query TEE: %s (rc=%d)", strerror(rc), rc);
		return rc;
	}
	...
	printf(
		...
		tee_version.impl_id, tee_version.impl_caps, tee_version.gen_caps);
